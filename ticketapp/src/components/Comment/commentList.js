...
const CommentList = ({ profileID, idt, onHandleUsage }) => {
                        ...
                        <Flex gap="large" vertical>
                            {comments.map((comment) => (
                                <div className="comment-section">
                                    <Comment
                                        author={<b>{getAuthor(comment.author)} </b>}
                                        content={<p>{comment.text}</p>}
                                        avatar={<Avatar src="https://joeschmoe.io/api/v1/random" alt="Han Solo" />}
                                    >
                                    </Comment>

                                    {profileID === comment.author && <Button onClick={() => handleEdit(comment.id)}>Edit</Button>}

                                    {profileID === comment.author && <Popconfirm
                                        key="delete"
